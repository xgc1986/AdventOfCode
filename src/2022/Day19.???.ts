...
export default class Day19 extends Puzzle<Input> {
    ...
    async run1(blueprints: Input): Promise<Solution> {
        ...
        totalQuality = (await this.run(blueprints[1], 24)) * 2;

        for (let i = 0; i < blueprints.length; i++) {
            break;
            const quality = (await this.run(blueprints[i], 24));
            console.log('Quality', quality);
            totalQuality += quality * (i + 1);
            //break;
        }
    ...
    private async run(blueprint: Blueprint, duration: number): Promise<number> {
        ...
        for (let minutes = 0; minutes < duration; minutes++) {
            ...
            let turnsToClay = Math.ceil((blueprint.clay.ore - totalOre) / oreProduction);
            let turnsToOre = Math.ceil((blueprint.ore.ore - totalOre) / oreProduction);

            if (clayProduction > 0) {
                turnsToObsidian = Math.max(
                    Math.ceil((blueprint.obsidian.clay - totalClay) / clayProduction),
                    Math.ceil((blueprint.obsidian.ore - totalOre) / oreProduction)
                );
            }

            if (obsidianProduction > 0) {
                turnsToGeode = Math.max(
                    Math.ceil((blueprint.geode.obsidian - totalObsidian) / obsidianProduction),
                    Math.ceil((blueprint.geode.ore - totalOre) / oreProduction)
                );
            }
            ...
            if (iCanBuy && turnsToObsidian <= 0) {
                ...
                if (obsidianProduction > 0) {
                    const newTurnsToGeode = Math.max(
                        Math.ceil((blueprint.geode.obsidian - totalObsidian) / (obsidianProduction + 1)),
                        Math.ceil((blueprint.geode.ore - (totalOre - blueprint.obsidian.ore)) / oreProduction)
                    );
            ...
            if (iCanBuy && turnsToClay <= 0) {
                ...
                if (clayProduction > 0) {
                    const newTurnsToObsidian = Math.max(
                        Math.ceil((blueprint.obsidian.clay - totalClay) / (clayProduction + 1)),
                        Math.ceil((blueprint.obsidian.ore - (totalOre - blueprint.clay.ore)) / oreProduction)
                    );
            ...
            if (iCanBuy && turnsToOre <= 0) {
                ...
                if (obsidianProduction > 0) {
                    const newTurnsToGeode = Math.max(
                        Math.ceil((blueprint.geode.obsidian - totalObsidian) / obsidianProduction),
                        Math.ceil((blueprint.geode.ore - (totalOre - blueprint.ore.ore)) / (oreProduction + 1))
                    );
                ...
                if (doIt && clayProduction > 0) {
                    const newTurnsToObsidian = Math.max(
                        Math.ceil((blueprint.obsidian.clay - totalClay) / clayProduction),
                        Math.ceil((blueprint.obsidian.ore - (totalOre - blueprint.ore.ore)) / (oreProduction + 1))
                    );
                ...
                /* check clay ?*/
                const newTurnsToClay = Math.ceil((blueprint.clay.ore - (totalOre  - blueprint.ore.ore)) / (oreProduction + 1));
